...
`stat_scalpcontours` creates an interpolated surface for an irregular
set of x-y coordinates, as is typically required for a topographical EEG
plot, and then calculates contours.
...
- breaks:
  ...
  Numeric vector to set the contour breaks. Overrides binwidth and bins.
  By default, this is a vector of length ten with pretty() breaks.
