...
Here we show how to fit a linear model to a single subject's data. We'll use the data from the LIMO EEG dataset[^1] - [S1](https://datashare.is.ed.ac.uk/handle/10283/2189?show=full)
...
An alternative parameterization would be to remove the intercept term. The function would then return separate coefficients representing `Face A` and `Face B`. This would be particularly helpful if you are planning to take these coefficients forward to a second-level analysis.
```{r}
